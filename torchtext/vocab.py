...
def load_word_vectors(root, wv_type, dim):
    ...
    if cm is not None:
        ...
        for line in trange(len(cm)):
            entries = cm[line].strip().split()
            length = len(entries)
            if length <= v_dim:
                print(entries[0].decode("utf-8")  + " : dimenison less than " + str(v_dim))
                continue
            word, entries = b' '.join(entries[0:length-v_dim]), entries[length-v_dim:length]
            if wv_size is None:
                wv_size = v_dim
            try:
...
class Vocab(object):
    ...
    def load_vectors(self, wv_dir=os.getcwd(), wv_type=None, wv_dim=300,
        ...
        self.unk_init = unk_init
        wv_dict, wv_arr, self.wv_size = load_word_vectors(wv_dir, wv_type, wv_dim)
        v_pre =self.set_vectors(wv_dict, wv_arr, lang)
        return v_pre

    def set_vectors(self, wv_dict, wv_arr, lang):
        v_pre = 0
        if lang is not None:
        ...
        for i, token in enumerate(self.itos):
            ...
            if wv_index is not None:
                ...
                # run lemmatization
                if lang is not None:
                    lem_token = stemmer.stem(token)
                    lem_wv_index = wv_dict.get(lem_token, None)
                    if lem_wv_index is not None:
                        v_pre += 1
                        self.vectors[i] = wv_arr[lem_wv_index]    
